...
cache busting. `SpaCacheMiddleware` (`core/channels/spa.py`) serves `/assets/*` as
`Cache-Control: public, max-age=31536000, immutable` and every other non-`/api/` path —
`/`, `index.html`, SPA-fallback routes — as `no-cache, no-store, must-revalidate`, so a
