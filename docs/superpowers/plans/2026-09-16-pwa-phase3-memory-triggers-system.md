...
The credential form is a real `<form>` with a `type="submit"` Save button, so Enter in a field saves; `SystemBench.test.tsx`'s every-button-is-`type="button"` guard becomes "unless it is a `submit` inside a form", which is the invariant it was always after. The form **sends only non-empty fields** (`validate_credential_body` stores field-by-field, so an omitted field keeps what is there while `""` would write an empty secret over a working one), **empties itself once `savedAt` is set** but not on a refusal, and offers `field.default` **only where nothing is stored**.
