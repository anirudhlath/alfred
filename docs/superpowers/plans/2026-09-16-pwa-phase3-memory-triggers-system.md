...
6. **Fire-and-forget controls never move the row.** Trigger enable/disable, fire, drain and run consolidation all return `{"status":"queued"}`. *(Corrected in task 11: this decision also listed **Save & test**, and the code does not. `PUT /api/integrations/{name}/credentials` answers `{"status": "ok"}` — `core/channels/web_server.py:678` — and the button then re-`GET`s `…/status`, so it is a confirmed direct write followed by a probe, and it belongs with DND and ending a session in the exceptions below. It still does not move on the tap: the visible beat is the re-read, not a queue.)* The control's own label becomes the status word, a note in `--accent` says when it was queued, and the row keeps its old state until a fresh read says otherwise. DND set/clear and ending a session are the two exceptions — direct writes the server confirms — and those do move.
...
|---|---|---|---|---|
| 1 | Episodic, no results | `best score 0.31 · threshold 0.55 · 128 hot, 1 204 cold searched` | `Nothing close enough to "<query>".` over `searched by meaning · the server does not report what it rejected` *(corrected in task 11: this cell said `nothing scored above the server's threshold`, which claims a threshold the response has no word for — there is none to report, and `MemoryBench.tsx:174` ships the sentence quoted here)* | `recall()` returns matches only. The score of the best *rejected* row, the threshold and the corpus sizes are not in the response and are not on any endpoint. Printing them would be invention. |
...
| 11 | System › Quiet | three DND states | same three, plus the footnote `A meeting in your calendar can also quiet Alfred; that is not shown here.` *(corrected in task 11 to the shipped copy — capital `A`, full stop; the same sentence is quoted correctly in task 8's own spec below)* | `overview.dnd` is the raw manual key. `DNDChecker` also honours calendar DND and lazily expires a stale manual window, and neither is visible over HTTP — so "off" here can be wrong. §5.2's whole point is that we say so. |
...
**Files:** Create `web/src/workshop/SystemSections.tsx`, `SystemSections.test.tsx`, `web/src/workshop/IntegrationRow.tsx`, `IntegrationRow.test.tsx` (the services row is 253 lines on its own; `SystemSections.tsx` lands at 425 without it). Modify `SystemBench.tsx` to render them, `web/src/lib/system.ts` + `system.test.ts` for the 404 note and the `1 streams` singular, and `web/src/workshop/useSystem.ts` + its test — **the plan forgot a seam for `Sign out on this device`**: task 7 shipped no logout, so `lib/system.ts` gains `logoutSession()` and the `credentials` slice gains `signOut` / `signingOut`.
...
Rows 56 px: `device_name`, then `sessionMeta(s, now)` in mono `.t-meta-strong` — **and `now` ticks, it does not freeze at mount.** These two lists span weeks; a Workshop left open across midnight would print last night's `signed in 23:50` as tonight's — the handoff's `passkey · pwa · signed in 07:02 · 192.168.1.24`. On the right, **`current`** for your own (a disabled label, not a button — you do not "end" the session you are using from a list) and **`End`** in `--accent-text` otherwise, ≥44 px. After ending: the row recedes to `--fg2` (not opacity .5 — see the Triggers task) and reads `ended 21:15 · applied`, then disappears on the re-read. Empty: `No other sessions.` (Services and Identity needed empty copy the plan never gave: `No connected services.` and `No passkeys registered.`) **Every empty state in these four sections is gated on a landed, successful read** — the hook hands each section `[]` with `error: null` while its query is pending, so an ungated sentence claims the house is empty before the server has answered and goes on claiming it under a 503's own error note. Three of these five sentences can never be true of a signed-in reader, which is how they were caught.

### `ServicesSection({ integrations })`
Rows: the name, `category · kind` in **`.t-meta-strong`** — `--muted` is 3.20:1 on `--surface` in light, and this line is the only thing separating `weather` the adapter from `weather` the registry service, so it is not decorative — then on the right the state word and an 8 px dot — `ok` in `--green-text`, `failed` in `--accent-text`, and **`unset`, `testing` and `queued` in `--fg2`, not the plan's original `--muted`**: `--muted` on `--surface` is 3.20:1 in light and the state word is the entire state of the row, so it cannot be decorative. Neither `testing` nor `queued` is a verdict, and spending the accent on a row that is merely busy would blunt `failed`. **`row.latency` is not drawn.** Task 7 derived it and no view consumed it; the handoff puts latency in the Health grid and nowhere else, and beside the state word it is a second reading of one probe — worse while a *save* is in flight, where the retained figure is a round trip measured against the credentials being replaced. Tapping a row expands a credential form built from `schema.fields`: one labelled input per field, 48 px tall, radius 10, **mono 14 px** (≥16 px if that fights iOS focus zoom — the zoom rule wins over the handoff's 14), `type="password"` for anything the schema marks secret (read the field shape in `core/channels/service_credentials.py` — do not guess the flag's name), and **never the value itself**, which the server does not send and must not. The field label and `help_text` are `.t-meta-strong` too, not `.t-label`/`.t-meta`: one is the visible label of a password field and the other is the `aria-describedby` target telling the reader how to fill it.

**The copy the all-or-nothing form settled on** — the plan's `configured[field] ? "saved" : ""` promised something the route will not do, so it is replaced:

| where | text |
|---|---|
| placeholder, stored field | `saved · retype to save` |
| standing note above the fields | `Every field is sent together · retype the ones already stored, or nothing is saved.` |
| beside a blank required field | `needed` |
| under the fields, while any is blank | `Still blank: <labels> · nothing is sent until every field is filled.` |
| under a `transient` field | `not stored · sent with this save and forgotten` |
| a save that never answered (20 s) | `No answer in 20 s · whether it was stored is unknown · try again.` |
...
Rows per passkey, the handoff's shape: `iPhone 15 Pro · passkey · registered 12 Aug · Face ID · this device` — i.e. `device_name`, then `credentialMeta(c, now)`, which carries the **whole** line — `passkey · registered 12 Aug · internal, hybrid · last used 07:02 · this device`, dropping any clause the server did not send. Task 7 first shipped only the transports and the last use, leaving this section to compose three clauses around it; that was overruled, because `sessionMeta` carries `passkey` and `this device` itself and two rows on one screen built by two different rules is the split the formatters exist to prevent. **`Add a passkey on another device`** in `--accent-text`, with `<n> registered` beside it; it mints a pairing code and shows it at 32 px, mono, letter-spaced, with `Pairing window closes 21:20 · enter this on the new device · not shown again once you leave` — the third clause is the plan's own "the note says so" requirement, which the first two clauses do not meet. The code is shown until the bench is left. Then **`Sign out on this device`**, drawn plain in `--fg` at weight 400 — `Alfred.dc.html:443-444` splits the two deliberately and the card's one accent belongs on *Add a passkey*, not on the destructive control. It is `POST /api/auth/logout`, and **any answer means signed out**: `core/identity/auth_routes.py:871-877` builds even the 503 response and then clears the session cookie unconditionally, so a failure branch that leaves the passkeys on screen saying "nothing changed" is false, and the Expired gate then rises 30 s later from nowhere. A refusal therefore reads `<detail> · signed out here either way · the house may hold the session until it expires`, and the re-read that raises the gate goes out on that branch too. **A second mint clears the first code before it asks**, or the reader is left holding a code the house has already replaced.
...
**Every refused write is reported beside the control that sent it, keyed as the busy flag beside it is keyed.** `errorText` carries no device or domain name, so one string per section says a row failed without saying which — and the next row's attempt erases it. `sessions.failed[id]`, `attention.failed[domain]` and `integrations.saves[name].error`; a section's own `error` is the *read* failure alone, and it outranks a write refusal because it is a fact about every row on the card rather than about one tap. `ended` and `saves` are pruned against the live list on every read, or they grow for the life of the Workshop. The credential save is the one write with a timeout (20 s) — nothing else in this tree bounds a request, and a hung `PUT` would leave the row reading `Testing…` and refusing every press until the Workshop is closed.
...
The credential form is a real `<form>` with a `type="submit"` Save button, so Enter in a field saves; `SystemBench.test.tsx`'s every-button-is-`type="button"` guard becomes "unless it is a `submit` inside a form", which is the invariant it was always after. The form **requires every `required && !transient` field, and refuses client-side before the PUT when one is blank, naming it.** The first build sent only non-empty fields on the theory that an omitted field keeps what is stored — true of storage, false of validation: `_validate_and_store` (`core/channels/web_server.py:419-431`) runs `validate_credential_body` (`core/channels/service_credentials.py:103-114`) first, and that 422s on any required non-transient field absent from the body whatever is in the keyring, with `required` defaulting to `True`. That made every multi-field service unrotatable — type a new token for `home-service`, press Save, get `422 · Missing required fields: ['url']` naming a field whose own placeholder said `saved`. So the placeholder must stop implying a field can be left blank, `required`/`aria-required` go on the input, and **`transient` must be surfaced** (it is never persisted, so `configured[key]` is always false for it and "blank means leave this one" is false — a later save silently drops it). The form **empties itself once `savedAt` is set** but not on a refusal, **clears on collapse** (a value that survives a collapse is a secret in unmounted-looking state with nobody watching it), and offers `field.default` **only where nothing is stored**.
...
- [ ] **Step 3: `docs/backlog/low/pwa-phase3-followups.md`** — one entry per deviation from the table above that a backend change would close, each stating the endpoint that would close it:
  ...
  14. **one owner for the deferred-queue drain.** `HeldBackSheet` keeps a local `queuedAt` (reset on each open) and `useSystem` keeps `maintenance.drainedAt`; they are independent, so the bench can read `queued 21:14 · …` while the sheet still offers `Drain queue now`. The server is idempotent — the route publishes an internal action and `drain_deferred` LPOPs until empty, so a second drain finds nothing — making this a narrative defect, not a delivery one. Lifting `drainedAt` to a shared owner (the Room, or a provider) is the fix.
  15. **Activity's scroll position dies on a tab trip.** Measured in task 10: `scrollTop` 420 → 0 and a fresh `<ul>` node, while `expanded` and `solo` survive in the hook — so a reader 400 rows down who glances at Memory returns with the row still open and the viewport at the top, and the anchor refs reset too, leaving the first live prepend after every return unanchored. A consequence of the benches unmounting, not of task 10: phase 2 unmounted `ActivityBench` the same way.
  16. recall stats on cold *browse* rows — the SELECT in `admin_api.py` omits `retrieval_count` and `last_retrieved`, so every cold browse row reads `never recalled`, and any cold row under the decay floor reads `decaying` whatever its real history. Search rows carry the stats; browse rows cannot.
  17. **Health's four stat cards are internal borders inside one container**, not the handoff's four radius-12 cards with an 8 px gap (`Alfred.dc.html:357`). The `SystemSection` frame is one card with rows in it, and every other section on the bench needs exactly that; four free-standing cards inside a section that is itself a card would be a card in a card, or a section that is not a `SystemSection`. The tension is the frame's, not the grid's — revisit if task 9's sections ever want a second frame.
