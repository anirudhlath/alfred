...
| `403 Invalid or expired pairing code` with a code you just minted | Ten wrong guesses from your client address inside five minutes refuse *that address* for the rest of its counter, correct code included (`alfred:webauthn:pairing:fails:{client_ip}`, TTL 300s). Wait it out — re-minting deliberately does not clear it. If the address in question is your reverse proxy's, `FORWARDED_ALLOW_IPS` does not list it, so every internet caller is sharing one budget; fix that first. A malformed header (anything but exactly six ASCII digits) gets the same 403 without being counted. |
