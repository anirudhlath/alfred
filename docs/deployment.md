...
- Strict mode is also what stops `alfredctl up` from auto-appending the container subnet
  (`172.16.0.0/12` on Docker, see [`containerization.md` §7](containerization.md)).
  `alfredctl up` prints a line saying so before it launches the container; plain
  `docker compose` prints nothing because it never appends the subnet to begin with.
...
| `403 Access restricted to trusted networks: <ip> is not trusted.` registering a passkey or writing credentials | **First read which IP it names** — it is always the peer the process saw, and the fix differs. (Anonymous callers get only the IP; the env-var hint is appended for signed-in callers.) **(a) Your device's own LAN address, no proxy in the path:** that subnet isn't trusted — add it to `ALFRED_TRUSTED_NETWORKS`. **(b) The proxy's address, or the Docker bridge gateway, from a device that *is* on the LAN or tailnet:** the gate is judging the proxy because `FORWARDED_ALLOW_IPS` doesn't list it — put that address in `FORWARDED_ALLOW_IPS` (never in `ALFRED_TRUSTED_NETWORKS`) and restart. Without strict mode you get the *silent* version instead: the proxy's RFC1918 address is trusted by default, so every internet caller passes. **(c) The bridge gateway, from a browser on the host, under `ALFRED_TRUSTED_NETWORKS_STRICT=1`:** working as intended — `alfredctl up` withholds the container subnet in strict mode and prints a line saying so as it launches (scroll back through that command's output; `docker compose` prints nothing here because it never adds the subnet at all). Register through the proxy or over Tailscale; don't add the subnet back. **Registering a passkey has a second route out of all three:** mint a pairing code from an already-signed-in device (`POST /api/auth/pairing`) and send it as `X-Pairing-Code` on both `register/begin` and `register/complete` — that passes from any network, without touching the trusted set. Credential writes have no such escape. |
| `403 Invalid or expired pairing code` with a code you just minted | Ten wrong guesses from your client address — a single IPv4 address or an IPv6 /64 — inside five minutes refuse *that client* for the rest of its counter, correct code included (`alfred:webauthn:pairing:fails:{bucket}`, TTL 300s; the key is the bare address for v4, the enclosing network for v6, e.g. `2001:db8::/64` — and an IPv4-mapped peer like `::ffff:203.0.113.5`, which a dual-stack listener reports for a v4 client, is unwrapped and keys on `203.0.113.5`, so grep for the plain address there). Wait it out — re-minting deliberately does not clear it. If the address in question is your reverse proxy's, `FORWARDED_ALLOW_IPS` does not list it, so every internet caller is sharing one budget; fix that first. A malformed header (anything but exactly six ASCII digits) gets the same 403 without being counted. |
| Boot log warns about `FORWARDED_ALLOW_IPS` | Three distinct warnings. **`…is '*'…`** — **never use this on an internet-facing host**: uvicorn's always-trust mode takes the *leftmost* `X-Forwarded-For` entry from any peer without validating it, so any caller picks the IP the trusted-network gate judges, and the gate stops being a perimeter. Set the proxy's own address instead. **`…is the loopback default…`** — unset or blank, so uvicorn trusts loopback only and `X-Forwarded-*` is ignored; harmless with no proxy, otherwise set it to the proxy's address. **`…entry '…' is not a valid IP or CIDR…`** — a hostname, a typo, or a `*` inside a comma-separated list (uvicorn only wildcards when the *whole* value is `*`); it becomes a literal no IP peer can match. Neither warning fires for a CIDR with host bits set (`172.18.0.5/16`) — Alfred validates with `strict=False` while uvicorn parses strictly, so that one is silently a never-matching literal. Use a bare address or a proper network. |
| iOS push stops arriving after this branch merges; `POST /api/devices/register` returns 401 | Device registration now requires a signed-in passkey session on top of the trusted network (see [`notifications.md` → Device Registration](notifications.md)). A client that registers with only a device token gets 401 and never lands in `alfred:push:devices`. **Re-verify push registration from the iOS client after merge** — check the token appears under `GET /api/admin/devices` — and update the client to carry the `alfred_auth` cookie if it does not. |
