...
- `EMBEDDING_BACKEND` selects how the embedding model runs, not which one: `sentence_transformers` (default) loads it in-process — one copy per service, and torch with it — while `openai` calls a shared OpenAI-compatible `/v1/embeddings` server at `EMBEDDING_HOST` (vLLM needs `--runner pooling`). Build providers via `build_embedding_provider()` (`core/memory/embedding_backend.py`), never by naming `SentenceTransformerProvider` in a service, and release them via `aclose()` — the ABC's default is a no-op, so closing can be unconditional. **Changing the model changes the vector width**, and both stores used to accept that silently: `FT.CREATE` returns "Index already exists" and `CREATE VIRTUAL TABLE IF NOT EXISTS` is a no-op, so vectors of the new width went into an index of the old one and recall returned nothing with no exception and no log. Both now compare dimensions and latch a proven mismatch, and the exception text **is** the recovery procedure, so read it instead of improvising. The blast radius differs by store: the hot store raises only from `add`/`search`/`count` (the three that call `ensure_index()`, so `delete`/`exists`/`update_metadata` keep working), while the cold store raises from `add`/`search`/`delete`/`exists`/`count` — every operation that opens the file, since they all reach `_ensure_schema()` via `_get_db()` — and because `EpisodicMemory.recall()` gathers hot and cold with `return_exceptions=False`, a latched cold store takes down the whole of recall, not just its archive half (`GET /memory/episodic?q=` → 503). Hot store: `FT.DROPINDEX idx:context` **WITHOUT** `DD`. `DD` deletes every `ctx:*` hash, and since `EpisodicMemory.write()` writes hot-only — entries reach cold SQLite solely via the Librarian's decay pass — that destroys every episodic memory not yet decayed, i.e. the most recent ones. Dropping the index alone keeps the hashes, but old-width entries stay unsearchable (`FT.INFO hash_indexing_failures`): semantic entries are rebuilt by the Librarian's next `reindex_semantic_files()` and routines are re-derived from YAML on restart, episodic ones have no re-embed path. Cold store: `DROP TABLE vec_episodic_content` + `vec_episodic_semantic` and `UPDATE schema_version SET version = 1`, then restart (the latch is per-process). That needs an interpreter with `sqlite_vec` loaded — not the `sqlite3` CLI, which is on the host that lacks the extension while the extension is in the image that lacks the CLI. Never delete the cold sqlite file: cold is the last stop, and `copy_to_cold_and_remove()` has already deleted the hot copy
